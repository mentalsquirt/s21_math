...
#define TEST_FUNC(name) \
  for (i = 0; i < (int)(name##_func.num_cases/sizeof(double)); i++) { \
    ...
    double y = name##_func.func_ptr(x); \
...
int main() {
  ...
  function_t cos_func = {cos, cos_cases, sizeof(cos_cases)};
  function_t sin_func = {sin, sin_cases, sizeof(sin_cases)};
  function_t tan_func = {tan, tan_cases, sizeof(tan_cases)};
  function_t acos_func = {acos, acos_cases, sizeof(acos_cases)};
  function_t asin_func = {asin, asin_cases, sizeof(asin_cases)};
  function_t atan_func = {atan, atan_cases, sizeof(atan_cases)};
